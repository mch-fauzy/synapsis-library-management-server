...
func (r *Repository) CreateUser(createUser *models.User) error {
	createdUser := r.PostgreSqlConn.Db.Create(createUser)
	err := createdUser.Error
	if err != nil {
		log.Error().Err(err).Msg("[CreateUser] Repository error creating user")
		return err
	}
...
func (r *Repository) GetUsersByFilter(filter models.Filter) ([]models.User, int64, error) {
	...
	err = r.PostgreSqlConn.Db.Transaction(func(tx *gorm.DB) error {
		...
		if len(filter.FilterFields) > 0 {
			for _, filterField := range filter.FilterFields {
				...
				case models.OperatorNot:
					...
					countQuery = countQuery.Where(filterField.Field+" != ?", filterField.Value)
				default:
					log.Warn().Msgf("[GetUsersByFilter] Unsupported filter operator: %s for field: %s", filterField.Operator, filterField.Field)
				}
			}
		}
		...
		if len(filter.Sorts) > 0 {
			for _, sort := range filter.Sorts {
				...
				default:
					log.Warn().Msgf("[GetUsersByFilter] Unknown sort order: %s for field: %s", sort.Order, sort.Field)
				}
			}
		}
		...
		if err != nil {
			log.Error().Err(err).Msg("[GetUsersByFilter] Repository error retrieving users by filter")
			return err
		}
		...
		if err != nil {
			log.Error().Err(err).Msg("[GetUsersByFilter] Repository error counting total users")
			return err
		}
